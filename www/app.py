...
async def init(my_loop):
    ...
    srv = await my_loop.create_server(app.make_handler(), config.server.host, config.server.port)
    logging.info('Server started at http://%s:%s' % (config.server.host, str(config.server.port)))
    return srv
